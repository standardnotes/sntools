...
describe('sn-tools', () => {
  ...
  describe('getNoteTitle()', () => {
    test('should strip only file extension, if single period in file name', () => {
      const fileName = 'test.md';
      const title = tools.getNoteTitle(fileName);
      expect(title).toBe('test');
    })

    test('should do nothing if no file extension', () => {
      const fileName = 'test';
      const title = tools.getNoteTitle(fileName);
      expect(title).toBe('test');
    })

    test('should strip only file extension, if multiple periods in file name', () => {
      const fileName = 'test 9.23.2023.md';
      const title = tools.getNoteTitle(fileName);
      expect(title).toBe('test 9.23.2023');
    })
  })
